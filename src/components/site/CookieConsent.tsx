...
export function CookieConsent() {
  ...
  const save = (prefs: Omit<Prefs, "t" | "essential">) => {
    try {
      localStorage.setItem(KEY, JSON.stringify({ essential: true, ...prefs, t: Date.now() }));
    } catch (error) {
      void error;
    }
    setOpen(false);
  ...
  return (
    <div
      ...
      className="fixed inset-x-0 bottom-0 z-[60] p-2.5 sm:p-4"
    >
      <div className="mx-auto max-w-4xl rounded-[1.75rem] border border-border bg-card p-4 shadow-elevated sm:rounded-3xl sm:p-6">
        ...
        {manage && (
          <div className="mt-3 space-y-2 rounded-2xl bg-secondary/60 p-3 sm:mt-4 sm:space-y-3 sm:p-4">
            <Toggle
              label={t.cookies.categories.essential.title}
              body={t.cookies.categories.essential.body}
              checked
              disabled
            />
            <Toggle
              label={t.cookies.categories.analytics.title}
              body={t.cookies.categories.analytics.body}
              checked={analytics}
              onChange={setAnalytics}
            />
            <Toggle
              label={t.cookies.categories.marketing.title}
              body={t.cookies.categories.marketing.body}
              checked={marketing}
